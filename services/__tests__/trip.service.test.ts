...
beforeAll(async () => {
  ...
  const trip = await createTrip({ name: 'Trip Service Integration Trip' });
  tripId = trip.id;

  // createTestUser signs in as the new user each time — re-sign in as organizer at the end
  coOrg = await createTestUser();
  const { error: e1 } = await admin.from('trip_participant').insert({
    trip_id: tripId,
    user_id: coOrg.id,
  ...
  const { error: e2 } = await admin.from('trip_participant').insert({
    trip_id: tripId,
    user_id: target.id,
...
afterAll(async () => {
  ...
  if (extraTripIds.length > 0) {
    await admin.from('trip_participant').delete().in('trip_id', extraTripIds);
    await admin.from('trip').delete().in('id', extraTripIds);
  }
...
beforeEach(async () => {
  const admin = getSupabaseAdmin();
  // Reset target to Participant before each test so role tests start from a known state
  await admin
